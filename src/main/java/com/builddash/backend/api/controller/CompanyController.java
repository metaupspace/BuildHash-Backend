...
public class CompanyController {
    ...
    @Operation(summary = "Get a company (members only)")
    public CompanyResponse get(@PathVariable UUID companyId,
                               @AuthenticationPrincipal AuthenticatedUser user) {
        return CompanyResponse.from(companyService.get(companyId, user.b2bMemberships()));
    }

    @PatchMapping("/{companyId}")
    @Operation(summary = "Update company profile (company ADMIN+)")
    public CompanyResponse update(@PathVariable UUID companyId,
                                  @Valid @RequestBody CompanyCreateRequest request,
                                  @AuthenticationPrincipal AuthenticatedUser user) {
        return CompanyResponse.from(companyService.update(companyId, user.userId(),
                user.b2bMemberships(), request.name(), request.gstNumber(),
                request.statementEmail(), request.businessTimezone()));
    }

    @PatchMapping("/{companyId}/status")
    @Operation(summary = "Suspend or activate a company (company ADMIN+)")
    public CompanyResponse updateStatus(@PathVariable UUID companyId,
                                        @org.springframework.web.bind.annotation.RequestParam CompanyStatus status,
                                        @AuthenticationPrincipal AuthenticatedUser user) {
        return CompanyResponse.from(companyService.updateStatus(companyId, user.userId(),
                user.b2bMemberships(), status));
    }
}
