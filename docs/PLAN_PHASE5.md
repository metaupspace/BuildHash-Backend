...
- **(d) Reschedule Logic (Modification Window)**: Slot swap and capacity adjustment for already-paid (`CONSUMED` lock) orders use dedicated `DeliverySlotServiceImpl.swapConsumedLock` and `releaseConsumedLock` methods (superseding `acquireOrSwapLock` which operates strictly on `ACTIVE` locks). The transition and lock swap/release MUST run inside a single `@Transactional` boundary (default `REQUIRED` propagation) — order update and slot swap commit or roll back atomically together.
...
- **(f) Idempotent Same-Status Delivery Webhook**: Same-status webhook redelivery returns 200 OK no-op; a new `DeliveryTrackingEvent` is appended only if coordinates (latitude/longitude) moved since the last recorded event.
- **(g) Post-Commit WebSocket Broadcast**: WebSocket broadcasts on `/topic/orders/{id}` fire strictly after transaction commit via Spring `TransactionSynchronization.afterCommit()`, wrapped in try/catch, and never fail or roll back the webhook's 200 OK response.
- **(h) 400 vs 409 Mechanics Split**: HTTP 400 is reserved for deserialization and syntactic/field validation errors (missing parameters, malformed JSON, out-of-range coordinates). HTTP 409 (`Conflict`) is returned for `InvalidOrderStateException` (illegal state machine transition jumps) and `ModificationWindowExpiredException`.
